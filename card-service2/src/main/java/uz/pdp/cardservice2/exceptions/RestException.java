...
import uz.pdp.cardservice2.utils.RestConstants;
...
public class RestException extends RuntimeException {
    ...
    private RestException(HttpStatus status, List<ErrorResponseDTO> errors) {
        this.status = status;
        this.errors = errors;
    }
    ...
    private RestException(String userMsg, int errorCode, HttpStatus status) {
        super(userMsg);
        this.errors = Collections.singletonList(new ErrorResponseDTO(userMsg, errorCode));
        this.userMsg = userMsg;
        this.status = status;
    }
    ...
    public static RestException restThrow(List<ErrorResponseDTO> errors, HttpStatus status) {
